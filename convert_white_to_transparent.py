#!/usr/bin/env python3
"""
Konvertiert weiße Hintergründe in PNGs zu transparent.
"""
...
def _remove_edge_connected_light_background(img, tolerance=40):
    ...
    # Nur randverbundene helle Flaechen entfernen, damit weisse Details im Icon bleiben.
...
def main():
    ...
    if not icons_dir.exists():
        print(f"Ordner nicht gefunden: {icons_dir}")
        return
    ...
    if not png_files:
        print(f"Keine PNG-Dateien gefunden in {icons_dir}")
        return

    print(f"Bearbeite {len(png_files)} Bilder...")
    success_count = 0

    for png_file in sorted(png_files):
        # Überspringe all_icons.zip
        if png_file.name == "all_icons.zip":
            continue
    ...
    print(f"\nFertig! {success_count}/{len(png_files)} Bilder erfolgreich verarbeitet.")
